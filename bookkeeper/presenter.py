...
class Bookkeeper():
    ...
    def update_budget_spent_column(self) -> None:
        ...
        for expense in all_expenses:
            ...
            # month
            m_start = datetime(year=year, month=month, day=1)
            m_finish_day = calendar.monthrange(year, month)[1]
            m_finish = datetime(year=year, month=month, day=m_finish_day)
            m_current = datetime.strptime(expense.expense_date, "%d-%m-%Y")
            if m_start <= m_current <= m_finish:
                spent_month += expense.amount
            # week
            monday = today - timedelta(days=today.weekday())
            expense_date = datetime.strptime(expense.expense_date, "%d-%m-%Y")
            if monday <= expense_date <= monday + timedelta(days=6):
                spent_week += expense.amount

        date_zip = zip([spent_day, spent_week, spent_month], ['День', 'Неделя', 'Месяц'])
        for spent_prd, prd in date_zip:
