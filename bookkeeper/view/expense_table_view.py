...
class QMessageBoxTypingFixed(QtWidgets.QMessageBox):
    Yes: QMessageBox.StandardButton
    Information: QMessageBox.StandardButton
    Ok: QMessageBox.StandardButton
    Cancel: QMessageBox.StandardButton
    Question: QMessageBox.StandardButton

    def __init__(self, parent: QWidget, icon: QMessageBox.StandardButton, text: str) -> None:
        super().__init__(parent=parent, icon=icon, text=text)  # type: ignore

class InputExpenseWindow(QtWidgets.QDialog):
    """ Window for entering information about new expense """
    safe_btn: QtWidgets.QPushButton
    ...
    def __init__(
        ...
        default_values: Union[dict[str, str], None] = None
    ):
        ...
        self.save_btn = QPushButtonWithSignals(msg_dict['save_button_text'])
    ...
    def _on_clicked_save_btn(self) -> None:
        ...
        if self._is_mandatory_filled():
            ...
            dlg = QMessageBoxTypingFixed(
                parent=self,
                icon=QMessageBoxTypingFixed.Information,
                text="Заполните поля 'сумма' и 'категория'"
            )
            dlg.setWindowTitle(' ')
            dlg.setStandardButtons(QMessageBoxTypingFixed.Ok)
            dlg.exec()
...
class MainTableWidget(QtWidgets.QWidget):
    ...
    input_win: Union[InputExpenseWindow, None]
    ...
    def __init__(self, parent: Union[QtWidgets.QWidget, None] = None):
        ...
        self.add_btn = QPushButtonWithSignals("Добавить")
        self.add_btn.clicked.connect(self._on_clicked_add_button)
        self.del_btn = QPushButtonWithSignals("Удалить")
        self.del_btn.clicked.connect(self._on_clicked_del_button)
        self.upd_btn = QPushButtonWithSignals("Редактировать")
    ...
    def _on_clicked_upd_button(self) -> None:
        ...
        if len(rows) == 0:
            ...
            row_data = {
                'expense_date': self.user_data[sel_row][1],
                'amount': self.user_data[sel_row][2],
                'category': self.user_data[sel_row][3],
                'comment': self.user_data[sel_row][4]
            }
    ...
    def _on_clicked_del_button(self) -> None:
        ...
        if len(idx) == 0:
            ...
            dlg = QMessageBoxTypingFixed(
                parent=self,
                icon=QMessageBoxTypingFixed.Question,
                text="Удалить выбранные записи?"
            )
            dlg.setWindowTitle('Удаление категории')
            dlg.setStandardButtons(QMessageBoxTypingFixed.Yes |
                                   QMessageBoxTypingFixed.Cancel)
            answer = dlg.exec()

            if answer == QMessageBoxTypingFixed.Yes:
                rows = set([i.row() for i in idx])
                pks = [self.user_data[i][0] for i in rows]
                self.remove_callback(pks)

    def register_add_callback(self,
